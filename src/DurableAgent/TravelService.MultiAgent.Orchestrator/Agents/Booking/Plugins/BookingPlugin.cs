...
namespace TravelService.MultiAgent.Orchestrator.Agents.Booking.Plugins
{
   public class BookingPlugin
   {
      ...
      public BookingPlugin(IServiceProvider serviceProvider)
      {
         ...
         _bookingServiceClient = serviceProvider.GetService<IBookingServiceClient>() ?? throw new ArgumentNullException(nameof(IBookingServiceClient));
      }
          ...
          )
      {
         ...
         Func<Dictionary<string, string>, Task<string>> callBookingConfirmationPlugin = async (inputs) =>
         {
            try
            {
               ...
               await _bookingServiceClient.InsertBookingAsync(userId,departureCity,destinationCity, fromFlightId, fromflightPrice, toFlightId, toflightPrice);

               await SendEmailAsync(userEmail, bookingMailConfirmationSummary);
            }
            catch (Exception ex)
            {
      ...
      public async Task SendEmailAsync(string emailId, string emailBody)
      {
         ...
         await _bookingServiceClient.SendEmail(new PostmarkEmail
         {
            From = from,
            Subject = subject,
            To = to,
